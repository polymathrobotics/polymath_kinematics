...
Kinematic models and forward projectors for differential-drive, bicycle (Ackermann), and
articulated vehicles. C++17 library with pybind11 bindings.

The interactive Kinematic Explorer lives in
[polymath_kinematics_python_tools](../polymath_kinematics_python_tools/).

## Installation

Needs Python 3.10+, a C++17 compiler, and CMake 3.15+. One `CMakeLists.txt` serves all three
paths; `find_package(ament_cmake QUIET)` selects the branch.

### Python wheel (uv)

```bash
uv sync    # from the repository root
uv run pytest polymath_kinematics/test/test_python_bindings.py
```

### ROS2 / colcon

Exports `polymath_kinematics::polymath_kinematics` plus the pybind11 module.
...
Self-contained: configures from this directory or from the repository root. Add
`-DCMAKE_DISABLE_FIND_PACKAGE_ament_cmake=ON` to force this path when an ament install on the
host would otherwise be found.
